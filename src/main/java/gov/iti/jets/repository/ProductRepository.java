...
public class ProductRepository extends AbstractRepository<Product> {
    ...
    public List<Product> findProductsByNameOrCategory( String productNameQuery, Category productCategory ) {
        TypedQuery<Product> queryByName =
                entityManager.createQuery( "SELECT p FROM Product p WHERE (p.name LIKE :name AND p.deleted = FALSE)", Product.class );
        TypedQuery<Product> queryByNameAndCategory =
                entityManager.createQuery( "SELECT p FROM Product  p WHERE (p.name LIKE :name AND p.category = :category AND p.deleted = FALSE)", Product.class );
        TypedQuery<Product> queryByCategory =
                entityManager.createQuery( "SELECT p FROM Product  p WHERE (p.category = :category AND p.deleted = FALSE)", Product.class );
    ...
    public List<Product> getPage( int pageNumber ) {
        TypedQuery<Product> query = entityManager.createQuery( "SELECT p FROM Product p WHERE p.deleted = FALSE ORDER BY p.id ASC", Product.class );
